...
- [ ] T305 Round the wall scoop's **flat bottom** where it emerges on each face (FR-043g, FR-043g1). The cut's bottom is a horizontal plane through the wall and ends in the wall's sawn cross-section: the face fillet follows the U's sides but the base reads as a square shelf. **The resolution is now specified** — the fillet **tapers to zero as it approaches the flat run** (FR-043g1), so it finishes inside the cut's own height. It was open this long because the requirement as written was unsatisfiable: a constant-radius fillet reaches a radius *below* the outline, and both other ways of absorbing that give something else up — clipping at the floor dip re-creates the sawn edge, and raising the flat bottom takes `wall/2` of depth off every compartment scoop (FR-043b5 now says the dip is not a fillet allowance). An exterior hole, which has wall below it rather than floor, keeps the untapered fillet (FR-043g2) and is already correct.
...
## Phase 32: What the finger-cut spec review turned up (FR-006a/b/c, FR-043a0, FR-043b1a, FR-039a, FR-047a/b/c)
...
- [x] T313 **Make the roll's rise reachable** (FR-043a0). Four numbers define an edge scoop and only three can be set: `TOP_ROLL_RISE_RATIO` is a module constant, so the one quantity FR-043c3 exists to separate cannot actually be varied by a caller. Thread it through `build_wall_scoop` and `FingerHoleBuilder` with the ratio as its default.
...
- [x] T315 [P] Fix the `finger_hole()` docstring: it says the hole "hangs from the rim", which contradicts FR-043b1 and the code it documents.
...
- [x] T321 Build a grip as **two circles joined by their internal tangent** (FR-043a7): a base circle sitting centred on the bottom of the cut, radius defaulting to half the width, and the roll at each end, with the tangent run as the flank. Centre the base circle on the axis (the corner construction puts it off-axis, which mirrors into two crossing arcs) and make the roll circular so the tangent is exact.
...
- [x] T323 **Grow the base circle as the cut shallows** (FR-043a7). Half the width is the largest round base and is the right size only while the cut can hold it: at 40mm wide and 10mm deep its arc covers barely half the half-width and the rest is a straight ramp — a shallow trapezoid with a dimple in it, which is what "still doing a weird looking shape" describes. Take `max(half-width, 0.9 × cap)`, the last tenth of the cap being what leaves a flank. It changes nothing at 15mm deep or more, which is the part already working.
