...
- [x] T247u Separate the **floor finger hole** from the edge scoop (FR-062). They were built from one profile, which put a flat-bottomed pan where a bowl belongs: a floor hole is a bore a piece is pushed *up* through, so its bottom stays tangent to the floor. `floor_bore_profile` is its own shape; `_sweep_through_wall` keeps the genuinely shared half (depth matching, face fillets, floor clip, side placement).
- [x] T247w Stop `r1` being silently zeroed (FR-058). The throat was capped to half the span first and `r1` given the remainder, so any scoop already at half the span lost its top roll entirely — the most visible part of the cut, gone on exactly the narrow compartments that need it most. The two now shrink together.
- [x] T247x Derive every scoop default from the cut instead of a constant (FR-059): `r1` and `r2` from the throat width, a finger hole's height from its radius. The 6mm default height came from the original's `depth_of_hole` — its *wall* depth, a different quantity — and made every hole a shallow nick regardless of the finger it was cut for.
- [x] T247y Hang exterior finger holes from the top of the **interior**, not the rim (FR-064). On a lidded box the lid band sits above the interior, so a rim-aligned hole started in solid material. Cap and slipover pass their own `interior_top`, since their bodies are already shortened and would otherwise take the allowance twice (FR-066).
...
- [x] T247ai Run a **compartment** scoop from the well floor to the box's **top face** (FR-067), not to the interior ceiling. The ceiling is where the well stops, not the wall: a lidded box's lid band stood above the cut as a step, with the r1 roll buried inside the wall. This was the step visible on both sides of Emberleaf's card box.
- [x] T247aj Default a scoop to the **shorter** wall (FR-068). Emberleaf's card boxes had theirs in the 98mm face; a card stack is lifted out across its narrow dimension, so it belongs in the 73mm one. `scoop_side` now stays `None` until resolved, so the default can apply.
- [x] T247ak Cap the floor dip at a small constant (FR-072). Sized as a share of the floor it reached 1mm — half of Emberleaf's 2mm floor — and showed as the cut eating into it. Its only job is to keep the cut's bottom face off the floor plane.
...
- [x] T247au Split the top roll into an **ellipse**: flare sets the mouth width, rise sets how gently the top turns (FR-057). Only the vertical extents compete for the wall's height, so a shallow wall keeps the full width and loses only curve — which is what the player card box needed, having lost its top curve entirely when the two were tied together.
- [x] T247av Widen the floor fillet to 0.65 of the throat half-width. It sits inside the throat, so it costs no width.
- [x] T247aw Give every **silhouette slot** a finger pull-out (FR-078/c2): a dish across the slot at half the piece's depth, rounded on every edge so a finger slides in from the surrounding floor. A slot cut to a piece's outline holds it exactly, which is what leaves nowhere to get a fingertip — the pull-out is part of the slot, not an extra. Per-slot depth, width, and opt-out.
- [x] T247ax Move the hinge **inside** the box's outline (FR-002r): the pin axis sits inside the back wall and is sunk so the barrel's crown is flush with the closed box's top. Phase 18 had recorded the protruding barrel as the one allowed exception to "a closed box is the size it declares" — but a box with a barrel hanging off it cannot be packed against its neighbours, and nothing tells the packer to reserve the room. Both hinged types now measure exactly their declared size in all three axes.
- [x] T247ay Relieve **each half out of the other** (FR-002t). Inside the box, the lid's knuckles occupy space the body's wall fills and the body's knuckles occupy space the lid's plate fills. `Closure` gained `body_cut` and `lid_cut`. Doing one side only is worse than a trap: the obvious symptom disappears while the other half stays welded — which is exactly what the first attempt produced.
- [x] T247az Carve the hinge out of the **interior mask** (FR-002s), following the original's `FilamentBoxInsideMask`. `interior_mask` is a per-type hook returning `None` for every type whose interior is simply its interior; the hinge types return the interior less the hinge's intrusion. `build_contents` clips the wells to it and leaves finger scoops alone, since breaching a wall is a scoop's job.
- [x] T247bb Give a **slipover sleeve corner finger notches** (FR-002u). A sleeve is otherwise a smooth box with nothing to grip; the original cuts two **diagonally opposite** corners just under the lid plate, so a pair of fingers pulls along its axis rather than twisting it.
...
## Phase 30: The corner indent's depth, and the U's two curves (FR-002q, FR-077)
...
- [x] T301 Stop splatting `_fit_radii` into `scoop_outline` in `build_wall_scoop` (FR-077). It returns `(flare, rise, r2)`; `scoop_outline` takes `(top_rounding, bottom_rounding, top_rise)`. The floor fillet was getting the top roll's rise — 1.6x the flare, a function of the *rounding* radius — so both curves of the U were driven by one number. On a 14mm scoop the bottom curve was 7.72mm where it should be 6.28mm.
- [x] T302 [P] Write test: the indent measures the lid's offset deep (SC-057), and the floor fillet tracks the throat radius while the top roll tracks the rounding radius (SC-054) — in `tests/test_pyboxbuilder/test_closures.py` and `tests/test_pyboxbuilder/test_finger_smoothing.py`

**Checkpoint**: Indents are the depth they claim; the U's two curves are sized from the two quantities that decide them.

---

## Phase 31: The lidless rim's inner edge, and the scoop's square base (FR-043f, FR-074)
...
- [ ] T305 Round the wall scoop's **flat bottom** where it emerges on each face (FR-074, FR-075). The cut's bottom is a horizontal plane through the wall and ends in the wall's sawn cross-section: the face fillet follows the U's sides but the base reads as a square shelf. **The resolution is now specified** — the fillet **tapers to zero as it approaches the flat run** (FR-075), so it finishes inside the cut's own height. It was open this long because the requirement as written was unsatisfiable: a constant-radius fillet reaches a radius *below* the outline, and both other ways of absorbing that give something else up — clipping at the floor dip re-creates the sawn edge, and raising the flat bottom takes `wall/2` of depth off every compartment scoop (FR-072 now says the dip is not a fillet allowance). An exterior hole, which has wall below it rather than floor, keeps the untapered fillet (FR-076) and is already correct.
- [ ] T305a [P] Write test: at the base of a compartment scoop the wall's face rolls into the cut on both faces — the cut is wider at the face than mid-wall *at the flat bottom*, as it already is at mid-height — and the cut still dips no more than 0.2mm below the well floor (SC-059).

- [x] T306 A wall scoop's solid is **taller than the height it was asked for**, and its placement ignores the difference. Measured: `build_wall_scoop(..., reach=14, wall_thickness=2)` returns a solid spanning z -0.20..16.00 — 2mm proud at the top (`RIM_OVERSHOOT_MM` plus the `rounding_edge` flare, `wall_thickness / 2`) and 0.20 below (the intended floor dip). `apply_finger_holes` then places it at `interior_top - reach`, which is the *nominal* height, so neither end lands where `reach` says: the top overshoots the rim by 2mm and the bottom sits 0.20 lower than intended. **Fixed by sizing the outline, not by moving the solid**: raising the whole scoop by the flare corrects the bottom and carries the top up with it, so the roll finishes a flare above the rim and the top face meets it sliced through mid-curve (FR-050 wants it tangent). `apply_finger_holes` now builds the outline `reach - flare` tall and hangs it from `interior_top`, which puts the roll's tangent at the rim and the flare's deepest point exactly `reach` below it; the lidded types' `top_limit` trims at the outline's own top, which is the same plane. Note the `no_lid_finger_holes` radius formula also carries a `+1` (`height - floor_thickness + 1`), which on a box where `reach` is capped by the interior depth puts the cut into the floor before the dip is even added.
...
## Phase 32: What the finger-cut spec review turned up (FR-006a/b/c, FR-051, FR-065, FR-039a, FR-047a/b/c)

**Purpose**: The 2026-08-15 review of every requirement covering finger cuts (see `checklists/requirements.md`) found two contradictions, three wrong statements and six missing rules. The spec and plan now carry the resolutions; this phase builds them.

**Goal**: Every finger cut in the library matches the revised spec, and each new rule has a test that fails without it.

**Independent Test**: A lidded box's exterior hole has no flat ceiling; a compartment scoop's base rolls onto both faces; a no-lid box with magnets has them clear of its holes.

- [x] T307 **A lidded box's exterior hole must finish its roll below the interior top** (FR-065). Today `apply_finger_holes` passes `top_limit`, which intersects the cut with a half-space at that plane: the result is a horizontal ceiling meeting the throat at 90° — the one square edge in the library a finger is guaranteed to touch, and exactly what FR-043 forbids. Shorten the cut instead: reduce the outline's height until the whole roll (its `rise`, from `_fit_radii`) fits under `interior_top`, and keep `top_limit` only as the guard that nothing reaches above that plane. A lidless box is unaffected — there the roll is already tangent to the rim.
- [x] T307a [P] Write test: probing down an exterior hole on a **sliding** box from the interior top, the opening narrows continuously and no horizontal face appears at that plane; the same hole on a no-lid box tops out tangent to the rim; both remove material over exactly the reach they were given (SC-058) — in `tests/test_pyboxbuilder/test_finger_smoothing.py`
- [x] T308 **Give FR-047's reach formula its reason in the code** (FR-047, SC-061). `hole_height = max(min(radius, height - ft - wt - 1), 2)` is right and reads as a fudge; the middle term is the depth at which the cut stops **a wall thickness plus a millimetre above the tray floor**, which is the strip the tray is lifted by. Name it in `no_lid_finger_holes` so it survives the next simplification, and drop the stale `finger_hole_size` wording from the docstring.
- [x] T308a [P] Write test: every automatic hole leaves at least `wall_thickness + 1` of tray wall below it, across a range of heights including one where the radius is not the binding term (SC-061).
- [x] T309 **Magnets move to the walls that carry no finger hole** (FR-039a). `_add_magnet_slots` pins the slots to FRONT/BACK while `no_lid_finger_holes` puts the holes in the longer pair, so on a box wider than it is long the pocket is cut inside the finger cut. Choose the magnet pair from the hole sides on the spec — the pair the holes did not take — and keep both slots opposing so FR-039's attraction is unchanged.
- [x] T309a [P] Write test: a no-lid box with magnets has zero shared volume between any magnet pocket and any finger cut, at footprints wider-than-long, longer-than-wide and square (SC-060) — in `tests/test_pyboxbuilder/test_all_box_types.py`
- [x] T310 **A polygon path box gets no automatic finger holes** (FR-047c). `PathBox.build_body` calls `add_no_lid_finger_holes` on every path, and the helper reads `width`/`length` — a polygon's bounding box — so the cut can land on a wall that is not there. Skip the automatic pair when `spec["path"]` is set; an explicit `finger_hole(side)` still works.
- [x] T310a [P] Write test: a path box with a polygon outline has no automatic holes and the same box without a path has the FR-047 pair (SC-064).
- [x] T311 **Report overlapping finger cuts** (FR-006c). Nothing detects them today, and the merged cut gives no sign of what made it. Check, at spec resolution: two cuts on the same wall whose mouths overlap, a cut overlapping a magnet pocket, a cut overlapping a lid track. Warn, naming each cut and its position, and build the geometry anyway (the plan's warnings table).
- [x] T311a [P] Write test: two overlapping holes on one wall warn once naming both; a hole clear of everything warns not at all (SC-063).
- [x] T312 [P] Write test: the **fingertip fits** — a 14mm-wide prism stood on the flat bottom of each default cut (compartment scoop, exterior hole, floor bore) is entirely inside the removed material (SC-062). This is the criterion that does not depend on which formula produced the cut.
- [x] T313 **Make the roll's rise reachable** (FR-051). Four numbers define an edge scoop and only three can be set: `TOP_ROLL_RISE_RATIO` is a module constant, so the one quantity FR-057 exists to separate cannot actually be varied by a caller. Thread it through `build_wall_scoop` and `FingerHoleBuilder` with the ratio as its default.
- [x] T314 **Refuse a cap or slipover box whose footprint cannot carry its corner cutouts** (FR-002m1). The height check (FR-002n) passes a 30 × 30mm box that then gets corner cuts meeting in the middle of every side. Raise the same style of error, naming the side, its length and the minimum.
- [x] T315 [P] Fix the `finger_hole()` docstring: it says the hole "hangs from the rim", which contradicts FR-064 and the code it documents.

**Checkpoint**: no square edge at the top of a lidded box's hole, no sawn edge at the base of a compartment scoop, no magnet inside a finger cut, and every rule in the revised spec has a test. **Partly reached** — T307–T315 are done; T305/T305a (the base's taper) remain, and are the only part of the revised spec not yet built.

- **Why T305 is still open, precisely.** The taper needs the face fillet's radius to vary along the outline — full over the throat, easing to zero at the flat run — and `offset_sweep` offsets its path **uniformly**: one radius for the whole ring. The three ways round it are all bigger than the task looks: sweep the fillet as a separate solid along part of the ring (the ring is closed, so an open path is not accepted), loft between two rings of different offsets (a chain of hulls, which FR-044e1 forbids because it convexifies the U), or roll the sweep by hand as a variable-offset surface. The last is the real answer and is a piece of geometry work in its own right.

---

## Phase 33: A grip you can trust, and a radius that is kept (FR-047, FR-054, FR-055)
...
- [x] T317 **Keep the corner radius; fit the outline around it** (FR-054). `_fit_radii` narrows it twice — a cap at `0.75 ×` the half-width (`MIN_FLAT_BOTTOM_RATIO`, which is the *compartment* scoop's flat-run rule) and then a proportional scale of rise **and** radius to fit the height. Asked for 20mm on the no-lid tray, it returns 14.4mm. Keep the radius, spend the **rise** first (down to a circular roll, never past it), and reduce the radius only against what is physically left. The common tangent already joins whatever circles it is given (§1a4), so the straight run absorbs the rest.
- [x] T318 **Default an exterior hole's base to round** (FR-054): its corner radius is half the cut's width — the largest circle there is — so the base is one curve into the tangents rather than a flat pan. A compartment scoop keeps its flat run: something rests on it.
- [x] T319 **Width and corner radius, settable independently** (FR-055) — `width` and `bottom_radius` on `FingerHoleBuilder` and through `build_wall_scoop`, either one changing the outline without the other.
...
## Phase 34: A shallow grip is a dish (FR-047, FR-052)
...
- [x] T321 Build a grip as **two circles joined by their internal tangent** (FR-052): a base circle sitting centred on the bottom of the cut, radius defaulting to half the width, and the roll at each end, with the tangent run as the flank. Centre the base circle on the axis (the corner construction puts it off-axis, which mirrors into two crossing arcs) and make the roll circular so the tangent is exact.
- [x] T321a [P] Write test: there is a flank of measurable length at every depth, it stands vertical once the cut is deep and lays over as it shallows, nothing creases where it meets either arc, and the base is the full round sitting on the bottom (SC-067).
- [x] T322 Solve the internal tangent **directly** rather than through `geometry.circle_circle_tangents`. Two reasons, and the second is what was shipped: the solver returns the external tangents too — which wrap both circles the same way and throw the flank wide of the cut — and it has nothing to return on the internal branch as the circles approach touching, falling back to a vertical that lies on neither arc. `beta = acos((r1 + r2) / d)` off the bearing between the centres gives both touch points outright.
- [x] T322a **First attempt, superseded, and worth recording.** Sizing the base circle to the radius at which the two circles *touch* — the flattest arc that reaches the cut's full width — produces an outline that is continuous, tangent at every join, and wrong: with the circles touching there is no flank at all, and the whole cut reads as one wobbling curve. The touching radius is a **cap**, not a size. The default is half the width, and the cap is never below that at any depth, so it binds only on an oversized request.

- [x] T323 **Grow the base circle as the cut shallows** (FR-052). Half the width is the largest round base and is the right size only while the cut can hold it: at 40mm wide and 10mm deep its arc covers barely half the half-width and the rest is a straight ramp — a shallow trapezoid with a dimple in it, which is what "still doing a weird looking shape" describes. Take `max(half-width, 0.9 × cap)`, the last tenth of the cap being what leaves a flank. It changes nothing at 15mm deep or more, which is the part already working.
- [x] T323a [P] Write test: at a tenth of the width in depth the base reaches at least three quarters of the half-width, the flank stays measurable at every depth, and the deep cut's numbers are unchanged (SC-069).
- [x] T323b **The rule had to reach the tray, and did not.** `apply_finger_holes` passed the half-width down as `bottom_rounding`, which reads as "the default" and is not: an explicit value counts as a *request*, so the growth in T323 never ran on any real box and the render came back byte-identical. The size is decided in `_fit_radii` and nowhere else; the caller says nothing unless the user did. Pinned by a test on the keywords the caller hands down, because nothing in the geometry shows the difference.

- [x] T324 **Narrow a grip to its depth** (FR-056): `throat = min(throat, outline height)`. The flank's angle at the rim follows the cut's aspect and nothing else — 45mm over 9mm arrives at 34°, the same width at 19mm deep arrives at 70° — so a shallow box gets a smaller grip rather than a stretched one. Every other lever was tried first: a bigger base flattens the flank, a smaller one flattens it further, and a bigger roll only softens the last millimetre.
- [x] T324a [P] Write test: no grip is wider across its throat than it is deep, a deep box's grip is unchanged, and the flank angle across box heights stays within about 25° instead of halving (SC-070).

- [x] T325 **A grip's roll keeps its tangency to the top face** (FR-052). `min(flare, height / 2)` clamped the *rise* alone, which lifts the roll circle's centre without shrinking the circle: it pokes above the rim, stops being tangent to the top face, and stops being the circle the cap and the tangent were solved against. The flank then joins a point that is not on the drawn arc and the outline runs sideways and down to reach it — a step in the shoulder, and the "weird box cutout" in the reported render. Radius and rise are one number; shrink both.
- [x] T325a Make `_tangent_join` **graceful where the circles touch**: return the touch point on the line of centres rather than a vertical that lies on neither arc. Exact touching is a real configuration (`depth = half-width + roll`), and there the flank is a point and the outline is still smooth.
...
- [x] T326 **A card well should notch, not bore** (FR-061). `MIN_WALL_SCOOP_DEPTH_MM` was inherited at 8mm, above the 6.5mm well of Emberleaf's player card box: it got the floor bore, which on a 10.5mm box shows as a nick in the rim a millimetre deep with the wall whole underneath. Lowered to 5mm — below a card well, above the 4mm tray the bore is for, and the number §1a9 of the plan already assumed.
- [x] T326a [P] Write test: a 6.5mm well gets the wall notch and a 4mm tray still gets the bore (SC-072).

- [x] T327 **Cut card wells through the floor** (FR-060). Port `FingerHoleBase`: a circle at the wall the side names, straddling it so it breaks through, running from below the base up to the wall's top, rolled where it emerges — into the floor, into the wall's top and onto both faces. `build_through_hole` in `compartments/finger_hole.py`.
...
- [x] T332 **Stop the overshoot at the floor** (FR-060). The inward overshoot that buys a constant section inside the wall was also running *below* the well's floor, where it bought nothing and scalloped the base inside the well across the cut's full width — 33mm of it, 2mm in from the wall, in the surface the box stands on. Clipped back to the wall's thickness below the floor; the bore is what opens the base and is unioned after.
- [x] T331 **One section through the wall** (FR-060). Rounding a single face left the cut *stepped*: `offset_sweep` holds its middle at the last offset its first rim reached, so the middle stayed at the path's full width and the rounded rim stepped in from it — 33mm at both faces, 30mm a fillet inside the outer one, a ridge round the inside of the hole. The sweep now keeps both rims and runs a fillet into the compartment (`inner_overshoot`), putting the inner transition in the void: one section, square inside, rolled outside.
- [x] T330 **Round only the box's outside, and keep the base flat** (FR-060). Two findings. The slot's ring ended a millimetre below the floor while its face fillet is half the wall, so the fillet rolled back up *inside* the base and left a curled lip round the hole — it now clears the base by the fillet as well. And `round_outer` decorated the **wrong face**: the sweep runs from the compartment side outward, so `bottom` is the inner face and `top` the outer, and the two were wired the other way about. Invisible while every scoop rounds both faces; now pinned by a test that asks for one.
- [x] T329 **The two curves inside the hole.** Its bore came out a 30-facet polygon — the ambient `fa` caps a circle at 30 segments however large it is — so it takes `rounding_facets` like every fillet, as the original takes a pinned 64. And its top corners met the wall's rim square where the original flares them by `rounding_radius`: that is the edge a hand meets picking the box up. The precision guard in `test_precision.py` learned that `rounding_facets` is precision-derived too, rather than being weakened.

- [x] T333 **Round a sliding box's top edge** by `wall_thickness / 4` (FR-043f1). Its lid lies in the channel, so the rails' outer edges are the top of the closed box and a hand runs along them — FR-043d's square rim assumes a lid that seals against it. One spec setting read by `build_shell`, so all three sliding types share it.
- [x] T333a [P] Write test: sliding, sliding-catch and card-library all measure a quarter-wall fillet at the top outer edge, and a lidded non-sliding type still measures square (SC-077).
...
## Phase 35: Senior Review — Safety Nets and the API

**Goal**: three shipped shape defects all had the same shape of cause — a wiring
mistake or a drift between the spec and the code, never the mathematics. These
tasks are the tests that would have caught each one, and the API changes that
stop the class of mistake from being expressible.

- [x] T334 **One measured-volume helper** (`tests/mesh.py`). `volume()` over an exported 3MF by the divergence theorem existed in three copies across the in-app scripts, drifting between them; the shared one has no import-time side effects, so the app preamble and a direct test both take it.
- [x] T335 **A spec-constant table** (`tests/test_spec_constants.py`). Eleven numbers the spec states in prose and the code states as a constant, checked in both directions. This is the exact drift that shipped the 8mm `MIN_WALL_SCOOP_DEPTH_MM` against a spec that had already been rewritten to 5mm (T326).
- [x] T336 **A geometric invariant harness** (`tests/invariants/`). 420 cuts — 6 half-widths x 7 depths x 5 flares — asserting the outline never doubles back, that joins carry direction across, that the shape stays inside its envelope, and that it is symmetric about its axis. Mutating any of the three shipped defects back in makes it fail.
- [x] T337 **A flag audit** (`invariants/test_every_flag_matters.py`). Each of the eleven switches on a cut has to change the cut, and the two faces have to differ from each other — which is what `round_outer` decorating the inner face (T330) would have failed.
- [x] T338 **Group the parameters into records.** `build_wall_scoop` took eighteen positional-or-keyword arguments and `_sweep_through_wall` seventeen, most of them `float | None`, several meaning the same thing under two names. Two frozen records — `CutProfile` (the outline's four numbers, FR-051) and `FaceTreatment` (how the cut is treated where it emerges) — take both signatures to eleven.
- [x] T339 **One field for the finger cut** (`Cut`). `finger_scoop` / `finger_cut` / `scoop_side` were three fields for one decision that had to agree: `finger_cut=SCOOP` did nothing without `finger_scoop=True`, and a `scoop_side` without a scoop was dropped in silence. `cut: Cut | None` says it once — `None` for no cut, a `Cut` (or a bare `FingerCut`, widened by `Cut.of`) for one — and carries the side and the shape with it.
- [x] T339a **Wire the record's measurements through.** `Cut`'s `width`/`depth`/`base_radius`/`mouth_flare`/`roll_rise`/`face_fillet` were accepted and dropped: a parameter that has no effect and raises no error is worse than one that does not exist. `carve.py` now folds them into the `CutProfile` and `FaceTreatment` it hands down, pinned by a handoff test rather than a shape test — the handoff is what was missing.
- [x] T340 **`finger_hole()` speaks widths** (FR-051). It took `radius` *and* `width`, one of them half the other, in a signature with eight keywords: an invitation to set the wrong one. `width` is now the parameter, alongside `base_radius`, `mouth_flare` and `face_fillet`; `radius` is kept for one release and raises a `DeprecationWarning`.
- [x] T341 **One owner for the dispatch** (`build_cut`). Which of three shapes a request becomes turns on two questions — the kind asked for, and whether the well is deep enough for a wall notch — and they were decided in two different modules, so a caller could satisfy one and miss the other. That is how the card boxes shipped with scoops (T327). Both live in `build_cut` now; `build_scoop` is a name for its SCOOP arm and decides nothing.
- [x] T342 **Split the finger-cut module** into the seam a cut actually has: `finger_outline.py` (the shape, in two dimensions — pure path maths, no solids), `finger_sweep.py` (taking it through a wall, and what each end does when it emerges) and `finger_cuts.py` (the three cuts and `build_cut`). One 1,544-line file was a long way to read to find out whether a number was about the shape or about the wall, and every defect that reached a render was one or the other. `finger_hole.py` re-exports all three so existing imports keep working; the library's own call sites take the specific module.
- [x] T343 **Renumber the finger-cut requirements** into their own block, FR-050–FR-079. They were lettered suffixes of FR-043 — `a0`, `a10`, `b1a`, `c3` — accreted in the order the defects were found, so the outline rules were spread across `a`, `c` and `e` and the numbering said nothing about the reading order. Grouped now as outline / which cut / where it goes / where it emerges / pull-outs, with a **was** column in the spec carrying every old id, since that is what the tasks above and the commits before this cite. 400 citations rewritten across the spec, plan, contracts, code and tests; FR-043 keeps only what is about a box's *edges* rather than its cuts (FR-043d, f, f1).
...
- [ ] T350 [P] Add 10 new enum members to `BoxType` in `pyboxbuilder/enums.py` (`SNAP_FIT`, `BAYONET`, `THREADED`, `DISPENSER`, `CARD_SHOE`, `DICE_TRAY`, `SLEEVE_DRAWER`, `CLAMSHELL`, `MODULAR_INTERLOCK`, `PRINT_IN_PLACE_HINGE`).
- [ ] T351 [P] Implement `SnapFitBoxBuilder` and `SnapFitBox` (FR-081: cantilever arms, 45° lead-in ramps, retention detents) in `pyboxbuilder/builders/snap_fit.py` and `pyboxbuilder/box/types/snap_fit.py`.
- [ ] T352 [P] Implement `BayonetBoxBuilder` and `BayonetBox` (FR-082: quarter-turn entry keyways, horizontal retention channels, tactile detents) in `pyboxbuilder/builders/bayonet.py` and `pyboxbuilder/box/types/bayonet.py`.
- [ ] T353 [P] Implement `ThreadedBoxBuilder` and `ThreadedBox` (FR-083: coarse 3mm pitch modified trapezoidal screw threads) in `pyboxbuilder/builders/threaded.py` and `pyboxbuilder/box/types/threaded.py`.
- [ ] T354 [P] Implement `DispenserBoxBuilder` and `DispenserBox` (FR-084: 40° gravity slide floor, single-token dispensing slot, vertical sight slot) in `pyboxbuilder/builders/dispenser.py` and `pyboxbuilder/box/types/dispenser.py`.
- [ ] T355 [P] Implement `CardShoeBoxBuilder` and `CardShoeBox` (FR-085: 20° angled draw well, low retaining lip, adjacent flat discard well) in `pyboxbuilder/builders/card_shoe.py` and `pyboxbuilder/box/types/card_shoe.py`.
- [ ] T356 [P] Implement `DiceTrayBoxBuilder` and `DiceTrayBox` (FR-086: 25mm+ rolling arena lid, 45° corner deflector fillets, recessed felt pad pocket) in `pyboxbuilder/builders/dice_tray.py` and `pyboxbuilder/box/types/dice_tray.py`.
- [ ] T357 [P] Implement `SleeveDrawerBoxBuilder` and `SleeveDrawerBox` (FR-087: hollow 4-sided outer sleeve, inner drawer tray with pull lip and rear push hole) in `pyboxbuilder/builders/sleeve_drawer.py` and `pyboxbuilder/box/types/sleeve_drawer.py`.
- [ ] T358 [P] Implement `ClamshellBoxBuilder` and `ClamshellBox` (FR-088: dual-tray 180° book fold, integrated spine hinge, perimeter closure catches) in `pyboxbuilder/builders/clamshell.py` and `pyboxbuilder/box/types/clamshell.py`.
- [ ] T359 [P] Implement `ModularInterlockBoxBuilder` and `ModularInterlockBox` (FR-089: perimeter dovetail interlocks and Gridfinity-compatible tiered base profiling) in `pyboxbuilder/builders/modular_interlock.py` and `pyboxbuilder/box/types/modular_interlock.py`.
- [ ] T360 [P] Implement `PrintInPlaceHingeBoxBuilder` and `PrintInPlaceHingeBox` (FR-090: monolithic 180° flat print-in-place captive cone-and-socket hinge joints with 0.35mm radial and 0.40mm axial clearance, front snap-fit catch) in `pyboxbuilder/builders/pip_hinge.py` and `pyboxbuilder/box/types/pip_hinge.py`.
- [ ] T361 Register all 10 new box types in `pyboxbuilder/box/registry.py`, `pyboxbuilder/builders/__init__.py`, and `pyboxbuilder/__init__.py`.
- [ ] T362 [P] Write unit tests for all 10 new builders and closure mechanisms in `tests/test_pyboxbuilder/test_extended_boxes.py`.
- [ ] T363 [P] Write geometric validation render tests in `tests/test_pyboxbuilder/render/test_extended_boxes_render.py`.
- [ ] T364 Add user guide documentation and `.. pythonscad-example::` snippets for all 10 new box types to `docs/box_types.rst`.
- [ ] T365 Pre-generate documentation STLs for the 10 new box examples via `scripts/generate_docs_stls.py`.
