...
1a6. **A scoop defaults to the shorter wall (FR-068).** A card stack is lifted out across its narrow dimension. In the long wall the cut is in the wrong place and the user reaches across the whole box for the cards, which is what Emberleaf's card boxes were doing. Overridable per compartment, since a box that holds something other than cards may want otherwise. For all card compartments (`cards()`, `is_card=True`, or card/deck naming), cutouts MUST always default to the short length of the compartment. When placed in a sliding box, the slide axis defaults along the long dimension so the open end matches the card's short wall without cutting into the lid grooves. If a box type's `preferred_scoop_side` conflicts by pointing to the long wall, card compartments override it with `default_scoop_side`.
...
| `CAP` / `CAP_PATH` | `cap_metrics`/`cap_body`/`cap_lid` | friction-fit cap over the rim | body stops a lid thickness short; supports rectangular footprints and arbitrary polygon paths (FR-018b) |
| `SLIPOVER` / `SLIPOVER_PATH` | `slipover_metrics` | sleeve down over the body | body inset all round, `foot` keeps the full footprint to seat on; supports rectangular footprints and arbitrary polygon paths (FR-018b) |
...
**Patterns cut through or inlay with multiple colors (FR-023, FR-024).** A pattern is a through-hole or surface-inlaid fill over the lid face, clipped twice: at the lid outline, and at the label area, which takes precedence. The catalogue is what `_PATTERN_FILLS` draws — `NONE`, `SQUARE`, `CIRCLE`, `HEX`, `DENSE_HEX`, `TRIANGLE`, `DENSE_TRIANGLE`, `OCTAGON`, `VORONOI`, `LEAF`, `LEAF_TESSELLATION`, `LEAF_VEINS`, `DICE`, `RING`, and `CHECKER` — and `build_pattern` **raises** for a member without a fill rather than falling back to a grid (T116/T117).
- *Shapes with holes in*: Patterns support annular/perforated shapes (e.g., `RING`, `DICE` with pips).
- *Hybrid through-holes (`through_holes=True`)*: When a pattern is inlaid into the surface in color (`inlay=True`), internal holes inside the shapes (or center apertures) penetrate completely through the lid, combining top-surface multi-color inlays with through-hole ventilation or interior viewing holes.
- *Multi-color patterns*: `PatternBuilder(colors=(...))` supplies a multi-color palette; patterns partition shapes and sub-elements across colors (e.g. alternating grid tiles, die bodies vs. pips, concentric ring bands).
- *Composite PatternResult*: Patterns can return `PatternResult` containing both through-hole geometry (`holes`) and colored inlays (`inlays`) partitioned by color, while single-solid patterns remain supported for full backward compatibility.
- *Through vs. Top-Layer Inlays (`through_inlay`)*: Multicolor parts can either be inlaid into the top surface (default `INLAY_DEPTH_MM = 0.6mm` / 3 layers) or extend completely through the full thickness of the lid (`through_inlay=True`), showing on both top and bottom faces. When `through_inlay` is a tuple of color indices (e.g. `(0,)`), specific parts (such as die face bodies or outer rings) extend through the lid while other parts (such as pips or inner discs) remain surface veneers (or vice-versa). In single-color export, through-inlays cut completely through the lid.

**Accents and colours, independently settable**: label text, frame top layer, and pattern color palette (`colors` / `pattern_color`) — each defaulting to a value distinct from the body colour. Patterns may assign different colours to different elements (FR-024).
...
| FR-018, FR-018a, FR-018b | Spacer Generation & Polygon Footprints (No-Lid, Cap, Slipover) | `paths.py`, `box/types/path.py`, `box/types/no_lid.py`, `box/types/cap.py`, `box/types/slipover.py` |
...
### Gravity Tile & Token Dispenser Mechanisms & Extraction Modes (FR-084)

The gravity dispenser tower architecture features an internal downward slide ramp and configurable tile extraction mechanisms (`DispenserExtractionMode`):

1. **Downward Internal Slide Ramp (`pyboxbuilder/box/types/dispenser.py`)**:
   - The ramp uses a clean 3D wedge prism sloping downwards from the back wall (`y = length - wt`, height `ramp_rise`) down to the floor at the front wall (`y = wt`, height `0`), so tiles slide by gravity toward the bottom dispensing slot rather than piling up backwards.

2. **Extraction Modes (`DispenserExtractionMode`)**:
   - `SCOOP` (default): A prominent front finger scoop (`scoop_radius`, default 14mm) combined with an underside floor cutout (`floor_scoop_depth`, default 12mm, `floor_scoop_width`, default 24mm). This allows players to pinch the bottom tile top-and-bottom with thumb and forefinger and slide it forward. A solid retaining cover bridge across the lower-mid region (`z < body_h * 0.5`) holds the remaining stack, while the upper sight slot (`z >= body_h * 0.5`) monitors stack height.
   - `TRAY`: Forward-extending landing tray shelf (`tray_depth`, default 20mm) with side guide rails and front stop lip, where the bottom tile slides out in front of the tower for direct pickup.
   - `REAR_PUSH`: Rear finger push slot on the back wall at floor level (`rear_push_width`, default 22mm) allowing a player to slide the bottom tile forward through the front slot from behind.
