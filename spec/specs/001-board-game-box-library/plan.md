...
1a9. **A compartment pull-out scoop curves over its own depth (FR-043c1, FR-043c2).** These are the scoops you drag a piece up and out of, so the roll wants to be the scale of the scoop rather than a token fillet: `r1` defaults to the scoop's **depth**, and the edge sweeps down into the well instead of turning a small radius and dropping straight.

   The second half is not done yet and is queued rather than claimed: where a scoop's mouth meets something **flat rather than a wall** — an adjacent compartment's floor, the top of a divider — it should blend into that surface rather than stopping at an edge. That needs the carve pass to know what is beside each compartment, which it currently does not; each well is built independently. The work is to give the layout a notion of a compartment's neighbours, then let a scoop that opens onto a flat area continue its roll across it.
