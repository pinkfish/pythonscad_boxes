...
- **FR-044f**: A **tray** — a compartment well holding loose pieces — MUST have both its vertical corners and its floor edges rounded, so a finger can sweep a piece up the curve and out. The default radius is **the well's own depth × 2/3**, capped so it cannot exceed the well's footprint or depth. Sizing it from the cutout rather than from the box is deliberate: what makes a piece easy to retrieve is the curve the finger follows, so a deep well needs a large sweep and a shallow token tray a small one.
- **FR-044g**: Rounding MUST be **zero** for any compartment whose shape is dictated by what it holds — card slots, SVG silhouettes, and element packs. A card stack needs a flat floor and square corners to sit in, and a piece silhouette is reproduced exactly as authored (FR-045).
...
- **SC-032**: A tray well of depth D has corner and floor radii of D x 2/3 (capped by its footprint), while a card slot, an SVG silhouette well and an element pack all have radius 0.
