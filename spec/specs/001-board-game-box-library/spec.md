...
- **FR-014**: The library MUST generate hollow spacer boxes/trays to fill all open spaces and gaps in the 3D packing layout. For vertical gaps along the Z-axis, a spacer box MUST be generated if the gap height is >= 3mm. If the gap height is < 3mm, the adjacent box's height MUST expand to absorb the gap. Spacer boxes MUST be shown in the layout PDF.
