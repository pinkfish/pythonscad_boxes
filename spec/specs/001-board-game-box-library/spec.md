...
- **FR-043a**: A finger scoop's mouth MUST meet the top of the box as an **S-curve**, not as a single arc. The surface MUST leave the flat top face with zero curvature, roll over, inflect, and settle into the scoop's throat with zero curvature again — so the top of the box flows down into the scoop and back out the other side with no crease anywhere along the transition. A single quarter-round fillet is NOT sufficient: it matches tangents at both ends but its curvature jumps abruptly at each, leaving a visible line where the flat top starts to fall away and another where the fillet meets the throat.
...
- **SC-024**: A finger scoop's mouth is an S-curve: sampling the cut's width against height shows the widening start and finish gradually (zero curvature at the top face and at the throat), not the constant curvature of a single arc.
...
- Smoothness is judged by *curvature continuity*, not merely by tangency: a transition that matches tangents but jumps in curvature still shows a crease, which is why FR-043a calls for an S-curve rather than a fillet.
