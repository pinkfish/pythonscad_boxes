...
- **FR-014**: The library MUST generate hollow spacer boxes/trays to fill all open spaces and gaps in the 3D packing layout. The spacer box can be non-rectangular (derived from 2D polygon paths) to fill all empty space and make the box complete to the full extent of the game box. Spacer boxes MUST be shown in the layout PDF.
