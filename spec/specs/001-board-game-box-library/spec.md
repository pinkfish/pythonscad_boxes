...
3. **Given** a sliding lid box, **When** the box and lid are generated, **Then** the stop wall (opposite the open end) is square top to bottom and the lid's leading face is square to meet it — there is no wedge, taper or undercut at the closed end that the lid would have to be forced past.
3a. **Given** a sliding lid box asked for a catch, **When** the box and lid are generated, **Then** the catch is a **bump and dimple**: a dimple in the wall beside the outlet and a matching bump on the lid that drops into it as the lid closes — never a wedge.
...
- **FR-002d**: The sliding lid's **leading end** — the end that enters the box's open end first — MUST carry a slight **chamfer** (a lead-in bevel) so the lid starts into the dovetail grooves without catching on their mouths. The stop end and the dovetail flanks MUST stay square: the chamfer is a lead-in, not a roundover.
- **FR-002e**: A sliding-lid box MUST NOT carry a **wedge catch**. The stop wall (opposite the open end) MUST keep its full thickness from the channel floor to the channel opening, and the lid's leading face MUST be square, so the lid slides up to the stop and meets it flat. A tapered, dovetailed or undercut stop end is a wedge catch: the lid has to be forced past it to close and sprung back out to open, which loads the thin leading lip in exactly the direction 3D-printed layers split. Only the two **side** walls carry a dovetail (FR-002c); the ends carry none.
...
- **SC-048**: A sliding box's channel is the **same length at its floor as at its opening**, and so is the lid: sliced at the channel floor and at the channel opening, the closed end of each sits at the same coordinate along the slide axis. No wedge, taper or undercut at the stop end. The lid-to-groove clearance is configurable, defaulting to 0.1mm per side.
