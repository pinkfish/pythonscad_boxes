...
- **FR-043c1**: A **compartment pull-out scoop** MUST curve down into the compartment over the scale of the scoop itself, not with a token fillet. Its top roll (`r1`) MUST default to the scoop's own **depth** — the height it cuts — so the edge sweeps down into the well and a piece can be dragged up the curve, rather than meeting a small radius and then a straight drop.
- **FR-043c2**: Where a compartment scoop's mouth meets something **flat rather than a wall** — an adjacent compartment's floor, the top of a divider, an open area beside it — the scoop MUST transition smoothly into that surface rather than ending at an edge. A scoop that is generous on the wall side and abrupt where it meets a neighbouring flat is worse than one that is uniform.
...
- **SC-043**: A compartment pull-out scoop's top roll spans the scoop's depth, so the cut has no straight vertical run where a shallow fillet would leave one.
