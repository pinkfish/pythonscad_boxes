...
- **FR-044**: The rounding/smoothing radius MAY be small — a 1mm fillet (or similar small radius) is an acceptable default; it is not required to be large. The rounding radius MUST be configurable where the box type supports it (a per-box or per-compartment `rounding`/`chamfer` parameter, defaulting to ~1mm). Box types that do not support configurable corner rounding MAY apply a fixed small radius.
