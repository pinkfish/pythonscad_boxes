...
class AllBoxTypesCatchMatingMatrixTests(unittest.TestCase):
    ...
    def test_sliding_family_catches(self) -> None:
        for bt in (BoxType.SLIDING, BoxType.SLIDING_CATCH, BoxType.CARD_LIBRARY):
            ...
            for ct in (CatchType.BUMP, CatchType.WEDGE, CatchType.LOOP, CatchType.NONE):
                for axis in ("x", "y"):
    ...
    def test_pip_hinge_catches(self) -> None:
        ...
        for ct in (CatchType.WEDGE, CatchType.NONE):
            with self.subTest(catch_type=ct.value):
                spec = replace(SPEC, catch_type=ct)
                body = impl.build_body(spec)
                self.assertIsNotNone(body)
...
class BuilderCatchMatrixTests(unittest.TestCase):
    ...
    def test_all_relevant_builders_accept_catches(self) -> None:
        builders = [
            ...
            CapBoxBuilder(label="b4", catch_type=CatchType.LOOP, catch_size=1.1),
            CapPathBoxBuilder(label="b5", path=POLY_L, catch_type=CatchType.WEDGE, catch_size=0.7),
            ...
            HingeBoxBuilder(label="b8", catch_type=CatchType.BUMP, catch_size=1.2),
            FilamentHingeBoxBuilder(label="b9", catch_type=CatchType.LOOP, catch_size=0.8),
